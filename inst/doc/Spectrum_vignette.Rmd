...
Spectrum is a fast adaptive spectral clustering method for single or multi-view data. Spectrum uses a new type of density aware kernel that strengthens local connections in dense regions in the graph. It uses a recently developed tensor product graph data integration and diffusion procedure for integrating different data sources and reducing noise. Spectrum contains the classical eigengap or new multimodality gap heuristic for determining the number of clusters (K). Spectrum was developed to cluster single and multi-omic data for precision medicine and molecular biology. However, the method is sufficiently flexible so it can generalise well to a wide range of Gaussian and non-Gaussian structures with automatic selection of K.
...
4. Single-view clustering: Brain cancer RNA-seq clustering a range of K
5. Multi-view clustering: Brain cancer multi-omics
6. Single-view clustering: Non-Gaussian data, 3 circles 
7. Single-view clustering: Non-Gaussian data, spirals
8. Ultra-fast single-view clustering: Gaussian blobs II
9. Parameter settings
10. Code for heatmaps
11. Closing comments
12. References
...
There will be some cases where the user would like Spectrum to return clustering assignments each K in a range so the results can be compared with other variables such as, histology, survival time, etc. There is an extra parameter that can be used for this.
...
Then the sample assignments for each K can be extracted as follows. For the desired K replace the number with this K in the below code, this extracts the assigments from the appropiate element of the list.
...
- For a lot of noisy Gaussian data e.g. RNA-seq, we have a preference for method 1 which is the eigengap. 
...
For users looking to make a heatmap from the Spectrum assignment output, the following code will help. We just need to order the input data's columns according to the clustering assignments. Then, when you make the heatmap with samples as columns, do not cluster the columns because this has been done already, and use the assignments as an annotation track to visualise them.
